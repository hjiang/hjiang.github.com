...
首先，Clojure是LISP家族的一门语言，这意味着在语法上，Clojure和其他的LISP语言是非常相似的。比如这个Clojure 表达式
...
前面说到符号可以代表运行环境中的对象。Clojure里可以通过下面的语句把一个符号绑定到特定对象上：
...
`project.clj` 是Leiningen的项目定义文件，里面主要包含项目所依赖的库及插件的版本信息：
